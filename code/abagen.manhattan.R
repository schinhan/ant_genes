...
abagen = read.delim(abagenFile, sep='\t', header=T, stringsAsFactors=FALSE)
refseq = data.frame(fread(cmd=paste0("gzip -dc ", refseqFile), sep='\t', header=F, stringsAsFactors=FALSE))

# get info
message('Querying gene infos.')
info = data.frame(matrix(NA, nrow = nrow(abagen), ncol = 6))
names(info) = c('gene','chr','start','stop','symbol','description')
pb = txtProgressBar(min = 1, max = nrow(info), style = 3)
for (i in 1:nrow(abagen)) {
  setTxtProgressBar(pb, i)
  info$gene[i] = abagen$gene[i]
  tmp = refseq[grep(paste0("\\b",abagen$gene[i],"\\b"),refseq$V10),]
  if (nrow(tmp) == 0) { tmp = refseq[grep(paste0("\\b",abagen$gene[i],"\\b"),refseq$V13),] }
  if (nrow(tmp) == 0) { tmp = refseq[grep(paste0("\\b",abagen$gene[i],"\\b"),refseq$V14),] }
  if (nrow(tmp) > 0) {
    info$chr[i] = tmp$V9[1]
...
# merge refseq and df
df = left_join(abagen, info, by = 'gene')
...
manhplot = function(GWAS = NULL, pCol = NULL) {
  ...
  GWAS$GENE_COUNT = 1 # inserted line, actually requires clumping
  # if (annotationThresh == 'fdr') {
  #   GWAS$annotation[GWAS$P < sigFDR ] = 1
  ...
  manhplot = ggplot(data = subset(GWAS, P >= 10^-yend)) +
    ...
    # geom_text_repel(
    #   data          = subset(GWAS, !is.na(annotation) & P >= 10^-yend),
