...
# settings
abagenFile="results/perm.pvals.txt"
...
# run script
Rscript code/abagen.manhattan.R "${abagenFile}" "${refseqFile}" "${targetDir}" "${yend}" "${ysteps}" "${width}" "${height}"
...
# general settings
abagenFile='results/perm.pvals.txt' # file with header: gene p.alert p.control p.orient
...
# create plots
Rscript code/abagen.qqplot.R "${abagenFile}" "${targetDir}" "${prune}" "${drawCI}" "${xend}" "${xsteps}" "${yend}" "${ysteps}" "${width}" "${height}"
...
# run analysis
Rscript code/abagen.manh.qq.combine.R "${traits}" "${manhattanPlots}" "${qqPlots}" "${outputFile}" "${width}" "${height}"
