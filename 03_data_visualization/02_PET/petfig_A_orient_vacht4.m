...
deg_fsa5(:,1) = parcel_to_surface([0;zscore(ant_recep(:,targets(6)))], 'laus_250_fsa5');
deg_fsa5(:,2) = parcel_to_surface([0;zscore(ant_recep(:,targets(6)))], 'laus_250_fsa5');
...
subplot('position',[.28 .81 .16 .20])
    ...
    double(antpl(2,vl)),'EdgeColor','none');
    view(-90,0)
...
subplot('position',[.62 .81 .16 .20])
    ...
    double(antpl(2,vr)),'EdgeColor','none');
    view(-90,0); 
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    lighting phong; material dull; shading flat;
...
subplot('position',[.31 .13 .63 .65])
    scatter(ant_cons(:,2),ant_recep(:,targets(6)),40,'k','filled');l=lsline; l.Color='r'; l.LineWidth=2;
    xlabel('ANT Orienting'); ylabel('vAChT availability')
%% adjust colormaps
...
%% export            
set(gcf,'color','w');
export_fig('visualization/pet/petfig_A_orient_vacht','-pdf') 
